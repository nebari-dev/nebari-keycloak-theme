...
type DataTableProps<TData extends Record<string, unknown>> = {
  ...
  /**
   * Lets pointer and keyboard activation on a row follow the first link inside
   * its `data-row-primary` element. The link remains available to assistive
   * technology but is removed from sequential focus so the row is one stop.
   */
  clickPrimaryRowLinks?: boolean;
...
type NavigableTableRowProps = ComponentProps<typeof TableRow> & {
  clickPrimaryRowLinks: boolean;
};

/**
 * Promotes a row's primary link to a single row-level keyboard target. Keeping
 * the real anchor in the cell preserves its destination and link semantics,
 * while `tabIndex=-1` prevents the visually plain name from becoming a second
 * focus stop. Enter follows that same anchor; nested controls keep their own
...
function NavigableTableRow({
  ...
  const [primaryLinkLabel, setPrimaryLinkLabel] = useState<string>();

  useLayoutEffect(() => {
    if (!clickPrimaryRowLinks) {
      setPrimaryLinkLabel(undefined);
      return;
    }

    const primaryLink = rowRef.current?.querySelector<HTMLAnchorElement>(
      '[data-row-primary] a[href]',
    );
    if (primaryLink === undefined || primaryLink === null) {
      setPrimaryLinkLabel(undefined);
      return;
    }

    primaryLink.tabIndex = -1;
    const nextLabel = primaryLink.textContent?.trim() || 'row details';
    setPrimaryLinkLabel((current) =>
      current === nextLabel ? current : nextLabel,
    );
  }, [children, clickPrimaryRowLinks]);

  const navigable = primaryLinkLabel !== undefined;
  const followPrimaryLink = () =>
    rowRef.current
      ?.querySelector<HTMLAnchorElement>('[data-row-primary] a[href]')
      ?.click();

  return (
    <TableRow
      {...props}
      aria-label={navigable ? `Open ${primaryLinkLabel}` : props['aria-label']}
      className={cn(
        className,
        navigable &&
          'cursor-pointer rounded-md outline-none focus-visible:bg-muted/50 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset',
        clickPrimaryRowLinks &&
          '[&_[data-row-primary]_a[href]]:pointer-events-none [&_[data-row-primary]_a[href]]:text-inherit [&_[data-row-primary]_a[href]]:no-underline',
      )}
      onClick={
        clickPrimaryRowLinks
          ? (event) => {
      ...
      ref={rowRef}
      tabIndex={navigable ? 0 : props.tabIndex}
    >
      {children}
    </TableRow>
  );
      ...
      <Table aria-busy={loading || undefined} aria-label={ariaLabel}>
        ...
        <TableBody>
          {loading ? (
            ...
            rows.map((row) => (
              <NavigableTableRow
                clickPrimaryRowLinks={clickPrimaryRowLinks}
...
// Registry components intentionally export their feature bundle for consumers
// that need to compose a compatible TanStack table instance.
